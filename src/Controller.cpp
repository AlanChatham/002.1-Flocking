...
void Controller::update(ci::Vec3f* lanternTargets)
{
	// LANTERNS
	int i = 0;
	for( std::vector<Lantern>::iterator it = mLanterns.begin(); it != mLanterns.end(); ){
		if( it->mIsDead ){
			...
			if (lanternTargets[i] == Vec3f::zero()){
				it->update( mRoom->mTimeAdjusted, mRoom->getFloorLevel() );
			}
			else{
				ci::Vec3f direction = lanternTargets[i] - it->mPos;
				direction = direction / 30;
				it->update( mRoom->mTimeAdjusted, direction, mRoom->getFloorLevel() );
			}
			++it;
			++i;
		}
	...
	if( mRoom->isPowerOn() ){
		if( ( Rand::randFloat() < 0.00475f && mNumLanterns < 8 ) || mNumLanterns < 1 ){
...
void Controller::drawGlows( gl::GlslProg *shader, HeadCam cam)
{
	for( vector<Glow>::iterator it = mGlows.begin(); it != mGlows.end(); ++it ){
		shader->uniform( "alpha", it->mAgePer );
		shader->uniform( "color", it->mColor );
		shader->uniform( "mvpMatrix", cam.mMvpMatrix );
		it->draw( cam.mBillboardRight, cam.mBillboardUp );
	}
}
